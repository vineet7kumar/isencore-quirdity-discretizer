...
void DataRelations::relate_data() {
  ...
  /*
   * Postprocess to fix adjacencies at the next finer and coarser levels.
   */
  vox_vert_vec = fix_adjacencies(vox_vert_vec, num_levels,
      getdata_abstractor());
...
int* DataRelations::set_adjacent_vertex_indices(
    ...
    vertex& ver, voxel_and_vertex &vv_l) {
    ...
    for (int y_off = -1; y_off <= 1; y_off++) {
      for (int x_off = -1; x_off <= 1; x_off++) {
        ...
        if (!is_out_of_box) {
          ...
          /*
           * The 'index' of the adjacent vertex should be in the geometry and
           * belong to one of the incident voxels on the reference vertex.
           */
          if (abstract_data_level.getvertex_zyx_vector()[index]) {
            if (is_vertex_in_incident_voxel(index, ver, /*vv_l,*/width, height,
                abstract_data_level)) {
              adj_ver[i] = index;
...
//            std::cout<<"calling function " << child_vertex.incident_voxels[0];
...
void DataRelations::fill_vertices(DataAbstractor abstract_data_level,
  ...
  for (int i = 0; i < num_set_vertices; i++) {
     ...
     * a vertex
     *
     * NOTE: This method has to be called before set_adjacent_vertex_indices.
     */
    ...
    vertices[i].adjacent_vertices = set_adjacent_vertex_indices(
        abstract_data_level, width, height, depth, vertices[i], vv_l);
    /*
...
const bool DataRelations::is_vertex_in_incident_voxel(int adj_index,
    vertex& ref_ver, /*voxel_and_vertex &vv_l,*/int width, int height,
    DataAbstractor abstract_data_level) {
//  std::cout << "checking incidence";
  int* inc_voxs = ref_ver.incident_voxels;
  vox_coordinate voxel;
  const vox_coordinate* all_voxels =
      abstract_data_level.getvox_coordinate_array();
  for (int i = 0; i < 8; i++) {
//    std::cout<<inc_voxs[i];
    if (inc_voxs[i] >= 0) {
      voxel = all_voxels[inc_voxs[i]];
      for (int z_off = 0; z_off <= 1; z_off++) {
        for (int y_off = 0; y_off <= 1; y_off++) {
          for (int x_off = 0; x_off <= 1; x_off++) {
            if (adj_index
                == linear_index(width, height, z_off + voxel.z, y_off + voxel.y,
                    x_off + voxel.x)) {
              return (true);
            }
          }
...
voxel_and_vertex_vector DataRelations::fix_adjacencies(
    voxel_and_vertex_vector& vox_vert_vec, int num_levels,
    const DataAbstractor* abstract_data) {
  voxel_and_vertex_vector vv = vox_vert_vec;
//  for (int i = 1; i < num_levels; i++) {
//    vv[i] = fix_adjacency_next_finer_level(vv[i], vv[i - 1], abstract_data[i],
//        abstract_data[i - 1], i);
//  }
  for (int i = 0; i < num_levels - 1; i++) {
    vv[i] = fix_adjacency_next_coarser_level(vv[i], vv[i + 1], abstract_data[i],
        abstract_data[i + 1], i);
  }
...
voxel_and_vertex DataRelations::fix_adjacency_next_finer_level(
    voxel_and_vertex& vv_this, voxel_and_vertex& vv_next_finer,
    DataAbstractor abstract_data_this, DataAbstractor abstract_data_nf,
    int level) {

//  std::cout<<"Inside fix adj for nfl\n";

  int num_set_vertices_this = vv_this.num_set_vertices;
  int adj_index;
  /*
   * For each vertex at this level, for each adjacent vertex (at NFL) ,
   * if it does not belong to incident voxels at NFL, set it to -1.
   */
  vertex relative_child_vertex;
  int rcv_index = -1;
  ReadBinvox temp1 = getreader()[level - 1];
  int child_height = temp1.getBinvoxData()->height + 1;
  int child_width = temp1.getBinvoxData()->width + 1;
  for (int i = 0; i < num_set_vertices_this; i++) {
    rcv_index = get_relative_child_vertex(
        vv_this.vertices[i].relative_coordinates, vv_next_finer,
        abstract_data_nf, child_width, child_height);
    if (rcv_index != -1) {
      relative_child_vertex = vv_next_finer.vertices[rcv_index];
      for (int j = 0; j < NUM_ADJACENTS; j++) {
        adj_index = vv_this.vertices[i].adjacent_vertices_next_finer_level[j];
        if (adj_index != -1) {
          /*
           * Check if it should be -1
           */
          if (!is_vertex_in_incident_voxel(adj_index, relative_child_vertex,
              child_width, child_height, abstract_data_nf)) {
            vv_this.vertices[i].adjacent_vertices_next_finer_level[j] = -1;
          }
        }
      }
//    }
...
voxel_and_vertex DataRelations::fix_adjacency_next_coarser_level(
    ...
    int level) {
  /*
   * Check that the adjacent vertices at the next coarser level are in a voxel
   * that is in the geometry and that the voxel is the one that also contains
   * the reference vertex at the current level.
   */
  voxel_and_vertex fixed = vv_this;
  int num_set_vertices_this = vv_this.num_set_vertices;
  int adj_index;
  vertex relative_parent_vertex;
  int* incident_voxels_ncl = 0;

  ReadBinvox temp1 = getreader()[level + 1];
  int parent_height = temp1.getBinvoxData()->height + 1;
  int parent_width = temp1.getBinvoxData()->width + 1;
  for (int i = 0; i < num_set_vertices_this; i++) {
    incident_voxels_ncl = find_incident_voxels_ncl(
        vv_this.vertices[i].relative_coordinates, vv_this, vv_next_coarser,
        abstract_data_this, abstract_data_nc, level, i);
    for (int j = 0; j < 8; j++) {
      adj_index = vv_this.vertices[i].adjacent_vertices_next_coarser_level[j];
      if ((adj_index >= 0 && !abstract_data_nc.getvertex_zyx_vector()[adj_index])
          || !is_vertex_in_incident_voxel(adj_index, incident_voxels_ncl,
              parent_width, parent_height, abstract_data_nc)) {
        vv_this.vertices[i].adjacent_vertices_next_coarser_level[j] = -1;
      }
...
const bool DataRelations::is_vertex_in_incident_voxel(int adj_index,
    int* incident_voxels, /*voxel_and_vertex &vv_l,*/int width, int height,
    DataAbstractor abstract_data_level) {
  int* inc_voxs = incident_voxels;
  vox_coordinate voxel;
  const vox_coordinate* all_voxels =
      abstract_data_level.getvox_coordinate_array();
  for (int i = 0; i < 8; i++) {
...
int DataRelations::get_relative_child_vertex(
    relative_vertex_coordinate ver_this, voxel_and_vertex& vv_nf,
    DataAbstractor ad_nf, int child_width, int child_height) {
  int x_at_child = ver_this.x * 2;
  int y_at_child = ver_this.y * 2;
  int z_at_child = ver_this.z * 2;

  int ref_ver_index_at_nfl = linear_index(child_width, child_height, z_at_child,
      y_at_child, x_at_child);
  const int* set_vertices = ad_nf.getset_vertex_vector();
  int len = ad_nf.getnum_set_vertices();
  int i = 0;
  for (i = 0; i < len; i++) {
    if (set_vertices[i] == ref_ver_index_at_nfl) {
      break;
    }
  }
  if (i < len)
...
int* DataRelations::find_incident_voxels_ncl(
    relative_vertex_coordinate ver_this, voxel_and_vertex& vv_this,
    voxel_and_vertex& vv_next_coarser, DataAbstractor abstract_data_this,
    DataAbstractor abstract_data_nc, int level, int ind) {
  int* incident_vox_ncl = (int*) (malloc(sizeof(int) * 8));
  for (int i = 0; i < 8; i++) {
    incident_vox_ncl[i] = -1;
  }

  /*
   * There will be 8 incident voxels if x,y,z at ref level are all even.
   * There will be 4 incident voxels if 2 of x,y,z are even.
   * There will be 2 incident voxels if 1 of x,y,z is even.
   * There will be 1 incident voxel if none of x,y,z is even.
   */
  ReadBinvox temp = getreader()[level + 1];
  int ncl_vox_height = temp.getBinvoxData()->height;
  int ncl_vox_width = temp.getBinvoxData()->width;
  int ncl_vox_depth = temp.getBinvoxData()->depth;
  int _x, _y, _z;

  int* inc_voxels_ref_level = vv_this.vertices[ind].incident_voxels;
  vox_coordinate vox_this;
  for (int i = 0; i < 8; i++) {
    if (inc_voxels_ref_level[i] > -1) {
      vox_this =
          abstract_data_this.getvox_coordinate_array()[inc_voxels_ref_level[i]];
      _x = vox_this.x / 2;
      _y = vox_this.y / 2;
      _z = vox_this.z / 2;

      if (_x < ncl_vox_width && _y < ncl_vox_height && _z < ncl_vox_depth) {
        incident_vox_ncl[i] = linear_index(ncl_vox_width, ncl_vox_height, _z,
            _y, _x);
      }
    }
  }
  return (remove_duplicate_voxels(incident_vox_ncl));
}

int* DataRelations::remove_duplicate_voxels(int* voxels) {
  for (int i = 0; i < 8; i++) {
    for (int j = i + 1; j < 8; j++) {
      if (voxels[i] != -1)
        if (voxels[j] == voxels[i]) {
          voxels[j] = -1;
        }
    }
  }
